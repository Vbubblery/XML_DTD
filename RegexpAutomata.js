import {CH_SPLIT,CH_MATCH,STATE_MATCH,State} from './State'
...
//Define the precedence of opt
let getPrecedence = (opt) => {
  switch (opt) {
    case PLUS:
    case STAR:
    case QUESTION:
...
//classify according to the function of opt.
let isLeftOpt = (opt) =>opt === OR || opt === POINT;
let isOpt = (ch) => ch === PLUS || ch === STAR || ch === QUESTION || ch === OR || ch === POINT;
let isUnaryOpt = (ch) =>ch === PLUS || ch === STAR || ch === QUESTION;
let isParen = (ch) =>ch === PAREN_LEFT || ch === PAREN_RIGHT;
let isCh = (ch) =>!isOpt(ch) && !isParen(ch);
...
let toPostfix = (re) => {
  ...
  for (let i = 0; i < len; i++) {
    ...
    if (isOpt(ch)) {
      let opt = ch;
      let optPreced = getPrecedence(opt);
      let isLeft = isLeftOpt(opt);
      while (opts.length) {
        let opt1 = opts[opts.length - 1];
        if (isOpt(opt1)) {
          let opt1Preced = getPrecedence(opt1);
...
//Generate NAF accoding to the Postfix Expression of RegExp. (Thompson's construction)
let toNAF = (postfix) => {
  let ch;
  let s1;
  let s2;
...
let reg = (regexp, str) => {
  let postfix = toPostfix(regexp);
  let start = toNAF(postfix);
